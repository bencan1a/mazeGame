...
- Do not merge or approve PRs. That applies to you as an **author** — every
  agent writing code here. The one exception is the automated reviewer that runs
  on the PR itself (`.github/workflows/claude-review.yml`), which is the
  designated approver. Its criteria are in [`REVIEW.md`](REVIEW.md).
