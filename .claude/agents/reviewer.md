...
You are the local pre-flight review, run from a session before a PR goes up. The
separate automated reviewer in `.github/workflows/claude-review.yml` votes on the
PR itself and applies the same checklist, so a clean run here should mean a clean
run there.
