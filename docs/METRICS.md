...
| `shortOfTarget`, `forcedSingles`    | Pieces the cut-and-orient peel cut shorter than asked         | How much board quality the peel is giving up      |
...
`shortOfTarget` and `forcedSingles` are not derivable from a finished `Board` —
